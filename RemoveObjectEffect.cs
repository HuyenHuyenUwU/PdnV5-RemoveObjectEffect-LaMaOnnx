...
namespace LaMaInpaintProject
{
    ...
    public sealed class RemoveObjectEffect : PropertyBasedEffect
    {
            ...
            RenderArgs srcArgs)
        {
            ...
            // Debug output: write to a writable temporary location instead of a hard-coded project path
            string debugPath = Path.Combine(Path.GetTempPath(), "lama_debug.txt");
            ...
            {
                // Swallow any IO errors from debug logging so the effect still runs for users
            }
        }
